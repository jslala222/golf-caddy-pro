...
export async function GET(req: NextRequest) {
  ...
  const schedules = (data ?? []).map((r: any) => ({
    ...
    swapMemo: r.swap_memo ?? undefined,
    swapThanks: r.swap_thanks ?? undefined,
    swapThanksAmount: r.swap_thanks_amount ?? undefined,
    swapThanksDate: r.swap_thanks_date ?? undefined,
    swapThanksAddedToLedger: r.swap_thanks_added_to_ledger ?? undefined,
    title: r.title ?? '',
    memo: r.memo ?? '',
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await db.from('aone_pro_caddypro_schedules').upsert({
    ...
    swap_memo: s.swapMemo ?? null,
    swap_thanks: s.swapThanks ?? null,
    swap_thanks_amount: s.swapThanksAmount ?? null,
    swap_thanks_date: s.swapThanksDate ?? null,
    swap_thanks_added_to_ledger: s.swapThanksAddedToLedger ?? null,
    title: s.title ?? null,
    memo: s.memo ?? null,
