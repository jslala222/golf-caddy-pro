...
export async function GET(req: NextRequest) {
    ...
    const { data, error } = await supabase
        ...
        .select('phone, notification_hour')
        .eq('code', licenseCode)
        .single();

    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ phone: data?.phone ?? '', notification_hour: data?.notification_hour ?? 6 });
}
...
export async function PUT(req: NextRequest) {
    ...
    const { licenseCode, phone, notificationHour } = body;
    if (!licenseCode) return NextResponse.json({ error: 'licenseCode required' }, { status: 400 });
